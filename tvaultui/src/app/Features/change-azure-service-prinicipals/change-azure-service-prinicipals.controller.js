...
(function (app) {
    app.controller('ChangeAzureServicePrincipalsCtrl', function ($scope, $rootScope, Modal, $timeout, fetchData, $http, UtilityService, Notifications, $window, $state, $stateParams, $q, SessionStore, vaultUtilityService, ModifyUrl, AdminSafesManagement, AppConstant, $filter, filterFilter, orderByFilter, RestEndpoints) {
        ...
        var getAzureSvcaccInfo = function (azureSvcaccId) {
            $scope.isLoadingData = true;
            $scope.isSvcaccExpired = false;
            $scope.expiredNote = '';
                ...
                function (response) {
                    if (UtilityService.ifAPIRequestSuccessful(response)) {
                        ...
                        try {
                            ...
                            $scope.azureSecretsData = object.secret;  
        ...
        $scope.requestDataFrChangeAzureSvcacc = function () {
            $scope.isLoadingData = true;
            if ($stateParams.azuresvcaccData) {
                // Prefilled values when editing
                $scope.changeAzureSvcaccHeader = "EDIT AZURE SERVICE PRINCIPAL";
                $scope.isEditAzureSvcacc = true;
                try {                    
                    getAzureSvcaccInfo($stateParams.azuresvcaccData.userId);
                } catch (e) {
                    // To handle errors while calling 'fetchData' function
                    if ($rootScope.showDetails !== true) {
                        document.getElementById('addUser').value = '';
        ...
        $scope.getAzureSvcaccInfo = function (svcaccObj) {            
            $scope.azureSvcacc = svcaccObj;
            $scope.azureSvcacc.azureSvcAccId = svcaccObj.userId;  
            $scope.azureSvcacc.accountExpires = $scope.azureSvcacc.accountExpiresFormatted;
            $scope.svcInputSelected = true;
            $scope.isCollapsed = false;
            $scope.autoRotate = false;
            document.getElementById('ttl').placeholder="TTL in seconds (Max: "+$scope.azureSvcacc.maxPwdAge+")";
            $scope.isSvcaccExpired = false;
            $scope.expiredNote = "";
            if ($scope.azureSvcacc.accountStatus.toLowerCase() == "expired") {
        ...
        $scope.init = function () {
            ...
            $scope.appNameTableOptions = [];
            $scope.requestDataFrChangeAzureSvcacc();
            $scope.fetchUsers();
            $scope.fetchGroups();
        }
        ...
        $scope.addPermission = function (type, key, permission, editingPermission) {
            ...
            if (duplicate) {
                ...
                try {
                    ...
                    $scope.showInputLoader.show = false;
                    $scope.showNoMatchingResults = false;
                    var azureSvcaccName = $scope.azureSvcacc.azureSvcAccId;
                    var apiCallFunction = '';
                    var reqObjtobeSent = {};
                    var updatedUrlOfEndPoint = "";
                    switch (type) {
                        case 'users' :
                            apiCallFunction = AdminSafesManagement.addUserPermissionForAzureSvcacc;
                            reqObjtobeSent = {"azureSvcAccName": azureSvcaccName, "username": key, "access": permission.toLowerCase()};
                            if (SessionStore.getItem("username") == key) {
                                $scope.isLoadingData = false;
                                clearInputPermissionData();
                                $scope.errorMessage = "Owner permission for an Azure service principal cannot be changed!";
                                $scope.error('md');
                                getAzureSvcaccInfo(azureSvcaccName);
                                return;
                            }
                            break;
                        case 'groups' :
...
//
        ...
        var getUserDetails = function () {
            $scope.ownerName = $scope.azureSvcacc.userName;
            $scope.ownerEmail = $scope.azureSvcacc.owner_email;
        }
