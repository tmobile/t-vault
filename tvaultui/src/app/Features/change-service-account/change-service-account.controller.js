    ...
    app.controller('ChangeServiceAccountCtrl', function ($scope, $rootScope, Modal, $timeout, fetchData, $http, UtilityService, Notifications, $window, $state, $stateParams, $q, SessionStore, vaultUtilityService, ModifyUrl, AdminSafesManagement, AppConstant, $filter, filterFilter, orderByFilter, CopyToClipboard) {
        ...
        $scope.onboardSvcAccount = function() {
            try {
                ...
                var appID = '';
                var appTag = '';
                if ($scope.svcacc.appName != '' && $scope.svcacc.appName != undefined) {
                    appName = $scope.applicationName.name;
                    appID = $scope.applicationName.id;
                    appTag = $scope.applicationName.tag;
                }
                var onboardPayload = {
                        ...
                        try {
                            ...
                            if (response.data.data) {
                                ...
                                if (object.appName != null && object.appName != '' && object.appName != undefined) {
                                    $scope.svcacc.appName = object.appName + " (AppID: "+ object.appID + ", AppTag: " + object.appTag + ")";
                                }                                
                                if ($scope.svcacc.appName != '') {                                    
                                    $scope.appNameTableOptions = [{"type": $scope.svcacc.appName, "name": object.appName, "tag": object.appTag, "id": object.appID}];
                                    $scope.applicationName = $scope.appNameTableOptions[0];
                                }
                                $scope.isCollapsed = true;
                                hideUserSudoPolicy();
                            }
        ...
        $scope.openSvcAccDefaultEditWarning = function (size) {
            Modal.createModal(size, 'openSvcAccDefaultEditWarning.html', 'ChangeServiceAccountCtrl', $scope);
        };

        $scope.openSvcAccNoRotationEditWarning = function (size) {
            Modal.createModal(size, 'openSvcAccNoRotationEditWarning.html', 'ChangeServiceAccountCtrl', $scope);
        };
